...
def solve_for_middle_pressure_and_velocity(left_state_primitive, right_state_primitive, species):
    ...
    integral_curve_left = lambda pressure: velocity_left + 2 * sound_speed_left / (gamma - 1.0) * (
        1.0 - np.power(pressure / pressure_left, exponent)
    )
    integral_curve_right = lambda pressure: velocity_right - 2 * sound_speed_right / (gamma - 1.0) * (
    ...
    hugoniot_locus_left = lambda pressure: velocity_left + 2 * sound_speed_left / denominator * (
        (1 - pressure / pressure_left) / (np.sqrt(1 + beta * pressure / pressure_left))
    )
    hugoniot_locus_right = lambda pressure: velocity_right - 2 * sound_speed_right / denominator * (
        (1 - pressure / pressure_right) / (np.sqrt(1 + beta * pressure / pressure_right))
    )

    # middle velocity must be the same when computed from left state and right state
    middle_velocity_left = lambda pressure: (
        hugoniot_locus_left(pressure) if pressure >= pressure_left else integral_curve_left(pressure)
    )
    middle_velocity_right = lambda pressure: (
        hugoniot_locus_right(pressure) if pressure >= pressure_right else integral_curve_right(pressure)
    )
...
def mass_density_if_shock(pressure_middle, pressure, mass_density, species):
    ...
    return ((1.0 + beta * pressure_middle / pressure) / (pressure_middle / pressure + beta)) * mass_density
...
def form_rarefaction_solutions(primitive_state, species, discontinuity_location, is_left_going):
    ...
    velocity_rarefaction = lambda x, t: ((gamma - 1.0) * velocity + 2.0 * (shift(x, t) + sign * sound_speed)) / (gamma + 1.0)
...
def form_exact_solutions(left_state_primitive, right_state_primitive, species, discontinuity_location):
    ...
    ) = form_rarefaction_solutions(left_state_primitive, species, discontinuity_location, True)
    (
        mass_density_rarefaction_right,
        velocity_rarefaction_right,
        pressure_rarefaction_right,
    ) = form_rarefaction_solutions(right_state_primitive, species, discontinuity_location, False)
