  ...
  const double time)
{
  ...
  for (int i_low_fidelity_model = 0; i_low_fidelity_model < std::ssize(low_fidelity_csv_files_); ++i_low_fidelity_model) {
    ...
    const double total_fluid_energy = low_fidelity_operations[i_low_fidelity_model]->computeTotalEnergy(low_fidelity_state);

    writeLineToCSVFile(low_fidelity_csv_file, i_time_step, time, low_fidelity_field_energy, total_fluid_energy);
  }
}
}