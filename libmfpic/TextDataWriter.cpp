...
void writeHeaderToCSVFile(std::ofstream& csv_file) {
  ...
  csv_file << "# Time_Step Time Field_Energy Total_Fluid_Energy Total_Fluid_Kinetic_Energy" << std::endl;
}
  ...
  const double total_fluid_kinetic_energy)
{
  ...
  field_energy << " " <<
  total_fluid_energy << " " <<
  total_fluid_kinetic_energy << std::endl;
}
  ...
  const double time)
{
  ...
  writeLineToCSVFile(main_csv_file_, i_time_step, time, particle_field_energy, 0., 0.);
