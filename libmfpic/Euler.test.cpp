...
TEST(Maxwellian, MaxwellianValueAtMean1D)
{
  constexpr double number_density = 1.2e16;
  const mfem::Vector bulk_velocity{5.4, 4.7, 8.3};
  constexpr double temperature = 305.1;
  ...
  mfem::Vector bulk_velocity_x(1);
  bulk_velocity_x(0) = bulk_velocity(0);
  double val_at_mean = euler::evaluateMaxwellian(prim, bulk_velocity_x, species);
  double expected_at_mean =
...
TEST(IsotropicKappaDistribution, IntegratesToOnein3D)
{
  ...
  for (int i = 0; i < N; ++i) {
    ...
    for (int j = 0; j < N; ++j) {
      ...
      for (int k = 0; k < N; ++k) {
        v(2) = bulk_velocity(2) + (-L + k * dv);
        integral += euler::evaluateIsotropicKappaDistribution(prim, v, kappa, species,3);
      }
    }
  }
  integral *= (dv * dv * dv);
