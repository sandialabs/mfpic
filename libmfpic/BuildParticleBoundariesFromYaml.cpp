...
buildParticleBoundariesFromYaml(const YAML::Node& particles_node, int mesh_dimension) {
  ...
  if (particles_node["Boundary Conditions"]) {
    ...
    if (particle_bcs.IsSequence()) {
      ...
      std::string message = formatParseMessage(
        particles_node["Boundary Conditions"],
  ...
  std::shared_ptr<ParticleBoundaryFactory> default_boundary_factory;
  if (particles_node["Default Boundary Condition"]) {
    default_boundary_factory = createParticleBoundaryFactoryFromTypeNode(particles_node["Default Boundary Condition"]);
  } else {
    default_boundary_factory = std::make_shared<ReflectingParticleBoundaryFactory>();
  }
